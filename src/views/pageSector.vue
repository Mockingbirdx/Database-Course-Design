    ...
    <el-container>
      <el-header style="text-align: right; font-size: 12px; height: 120px">        
        <el-form :inline="true" :model="formInline" class="demo-form-inline"  ref="ruleFormRef" style="position: relative;top: 20px;" >
            ...
            <el-input v-model="formInline.eid" placeholder="例：124673" />
            </el-form-item>
            <el-form-item label="enode-name">
            <el-input v-model="formInline.ename" placeholder="例：A池刘果-HLHF" />
            </el-form-item>    
            <el-form-item>
...
const value = ref('')
const options =  ref(reactive([{
  value: '',
  label: ''
}]))
let optionData = ref(reactive([
    {
        value: '...',//sectorname
        label: '...'//sectorname
    }
...
const loadOptionData = () => {
  // Some code
  axios({ method: 'GET', url: '/tbcell/sectorname' })
    .then(resp => {
            if (resp.data) {
    ...
    console.log('optdata:',options.value)
}
